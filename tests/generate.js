...
const {generateRecord} = require('../src/generate');
...
test('table name is required', async (t) => {
  await t.throwsAsync(generateRecord(pool), {instanceOf: SQL.MissingRequiredArgError});
});

test('the black box looks good', async (t) => {
  const explicitPool = await generateRecord(pool, GenerateTest.table);
  t.deepEqual(explicitPool, expectedGenerated);

  SQL.setDefaultPool(pool);
  const defaultPool = await generateRecord(GenerateTest.table);
